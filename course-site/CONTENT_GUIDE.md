...
| 탭 | 내용 |
|---|---|
| 개념 이해 | 핵심 개념 카드 + 개념 시각화 요약 + 단축키 참조. 부가 설명은 `더보기` 내부 펼침으로 처리 |
| interaction | Canvas/SVG 인터랙티브 데모만 배치. 가능하면 Blender 실제 패널처럼 슬라이더, 숫자 입력, 체크박스로 조작하고, 보조 설명은 긴 문장 대신 작은 핵심 카드로 요약 |
| 언제 쓰나요? | 사용 사례 + 팁 |
| 퀴즈 | 3~4문항 객관식 |
```
...
[ ] videos URL이 Blender Studio 또는 Blender 공식 채널인가?
...
> 같은 저장 동작에서 `weeks/.../lecture-note.md`의 자동 연동 요약 블록도 함께 갱신됩니다.
...
강의노트 자동 연동 블록 갱신
    ↓
필요하면 같은 창에서 Notion 반영 또는 Notion 내용 먼저 가져오기 선택
```
...
- **원본 데이터는 `curriculum.js`**입니다. 웹은 이 파일을 바로 읽고, Admin 저장도 이 파일에 먼저 반영됩니다.
- **강의노트 연동**: `lecture-note.md` 전체를 통으로 덮어쓰지 않고, 자동 연동 마커 구간만 갱신합니다. 강의 본문은 직접 작성한 내용을 유지합니다.
- **저장 전 확인**: 저장 버튼을 누르면 먼저 비교 창이 뜹니다. 여기서 `웹 + 강의노트만 저장`, `웹 + 강의노트 + Notion 저장`, `Notion 변경 먼저 가져오기`, `취소` 중 하나를 고릅니다.
- **📤 Push (Admin → Notion)**: Notion 페이지의 기존 내용을 **모두 교체**합니다. Notion에서 직접 꾸민 포맷(색상, 콜아웃 등)은 유실될 수 있으므로, 내용이 확정된 후에만 Push하세요.
- **저장 기본값**: 상단 토글은 저장 창에서 어느 방향을 기본 추천으로 보여줄지 정합니다. 자동 저장을 바로 실행하는 용도는 아닙니다.
- **📥 Pull (Notion → Admin)**: 비파괴적입니다. 변경사항을 미리보기로 보여주고, 교수자가 확인 후에만 적용됩니다. "저장" 버튼을 눌러야 최종 반영됩니다.
- Notion 토큰이 없으면 동기화 버튼이 표시되지 않습니다.
- `curriculum.js`를 코드에서 직접 수정했을 때는 `python3 tools/sync_course_content.py`를 실행하면 강의노트 자동 연동 블록을 다시 맞출 수 있습니다.
