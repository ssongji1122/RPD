...
리서치 브리프 §3(단축키 표) 외 본문에서 키 입력 인용 시: `<span color="red">Shift+A</span>` 형식. Notion 본문 inline 한정. 코드 블록 안에서는 비적용.

상세: `course-site/CONTENT_GUIDE.md` §4-A.

## Blender 버전 변수

작성 직전 `~/.claude/projects/-Users-ssongji-Developer-Workspace-RPD/memory/user_blender_version.md` Read → `BLENDER_VERSION` 값 추출 (SoT).

- 브리프 헤더: `> 생성일: {YYYY-MM-DD} | Blender {BLENDER_VERSION} 기준`
- 본문 내 "Blender 5.0" 등 하드코딩 금지. 항상 변수 표기.
- 공식 문서 URL은 `https://docs.blender.org/manual/en/latest/...` 유지 (latest 안전).
- 버전별 차이가 핵심 정보일 때만 명시 (예: "Blender 4.0 이후 단축키 변경").

버전 바뀌면 메모리 파일 한 곳만 수정 → 이후 작성물에 자동 반영.
