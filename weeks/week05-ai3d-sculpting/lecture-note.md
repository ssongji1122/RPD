...
> 💡 **참고:** AI 메쉬 Remesh 심화(Voxel Remesh/Quad Remesh/QRemeshify/Mesh Cleaner 2)는 **Week 6 Step 1**에서 본격적으로 배워요. 오늘은 기본 Sculpt 흐름에 집중하세요.
...
| 4 | Sculpt 수정 후 **Edit Mode로 돌아갔을 때** 토폴로지가 엉망 | Dyntopo가 삼각형 메쉬를 생성 | Week 6 Step 1 Remesh에서 정리 예정. 지금은 저장만 해두기 |
| 5 | AI 메쉬가 너무 무거워 버벅임 | 고폴리 AI 메쉬 | Decimate Modifier로 기본 감량 (Ratio 0.3~0.5). Remesh 심화는 Week 6 |
...
- 예상 시간: ~3시간
- **참고:** Remesh 심화(Voxel/Quad/QRemeshify/Mesh Cleaner 2)는 Week 6 Step 1에서 다룹니다.
...
- Meshy 또는 Tripo에서 프롬프트 입력 후 생성 (구체적인 형용사를 넣을수록 결과가 좋아요)
- 다른 프롬프트로 한 번 더 생성해서 비교 (같은 주제라도 문장에 따라 결과가 달라요)
- .glb 파일 Blender에서 Import (File → Import → glTF (.glb/.gltf))
...
- Viewport Overlay에서 폴리곤 수 확인 (Statistics 켜기)
- Decimate Modifier 추가 후 Ratio 조절 (0.3~0.5 정도에서 형태 유지되는 지점 찾기)
- Ctrl+A로 Scale 정리 후 원점 확인 (Import 메쉬는 크기가 제각각이에요)
...
- F로 브러시 크기, Shift+F로 강도 조절 (큰 브러시로 시작해서 점점 줄이기)
- Draw 브러시로 표면 올려보기 (Ctrl 누르면 반대로 파내기)
- Grab 브러시로 형태 잡아 끌어보기 (큰 덩어리 잡을 때 유용해요)
- Smooth 브러시로 울퉁불퉁한 곳 정리 (Shift 누른 채로도 임시 Smooth)
...
- Clay Strips로 점토 덧붙이듯 형태 쌓기 (넓은 면 위에 층층이 쌓기)
- Crease로 홈이나 주름 선 파기 (관절, 눈, 입 라인에 활용)
- Inflate로 볼록하게 부풀려보기 (볼이나 근육 강조에 유용)

#### 5. AI 초안 완성 및 .blend 저장

AI Sculpt 흐름의 마지막 단계예요. Week 6 Remesh + Material 실습에서 오늘 파일을 사용합니다.

배울 것

- AI 초안 + Sculpt 마무리 흐름을 완성한다
- Week 6에서 사용할 .blend 파일을 올바르게 저장한다

체크해볼 것

- 최종 형태 Object Mode에서 확인 후 스크린샷 (Before/After)
- File → Save As → 파일 이름에 week05 포함해서 저장
- **Week 6 Remesh 심화는 다음 주 Step 1에서**: Voxel/Quad Remesh, QRemeshify, Mesh Cleaner 2
