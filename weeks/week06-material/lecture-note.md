...
> **Week 05 완성 모델을 Remesh로 정리한 뒤, Material을 입히는 단계**
>
> 이번 주는 두 파트로 구성됩니다:
> - **Part 1 — Remesh 정리 (Step 1)**: AI 메쉬의 지저분한 토폴로지를 Material 적용 가능한 상태로 정리
> - **Part 2 — Material & Shader (Step 2-6)**: 정리된 메쉬에 재질을 입히고 Shader Node 연결
>
> - **3D 모델 확보** (Week 05): AI 생성 + Sculpt 수정으로 로봇 형태 완성 → 오늘 Remesh 시작점
> - **Mesh 기본 정리** (Week 05): Decimate/Mesh Cleaner 간략 체험 → 오늘 Step 1에서 심화
...
- [ ] AI 메쉬를 Remesh(Voxel/Quad/Decimate/QRemeshify/Mesh Cleaner 2)로 Material 적용 가능한 상태로 정리할 수 있다
...
## Step 1 — AI 메쉬 Remesh 정리 (30분)

### 왜 Remesh가 필요한가?

AI 3D 생성 툴(Meshy/Tripo/Hyper3D)로 만든 메쉬는 토폴로지가 불균일하고 삼각형이 뒤엉켜 있어요. 이 상태로 Material을 입히면:

- 쉐이더가 깨지거나 이상하게 표시됨
- UV 전개가 불가능하거나 비정상
- Normal이 뒤집혀 빛 반사가 엉망

**Remesh는 "내 메쉬를 작업 가능한 상태로 만드는 필수 준비 단계"**예요.

### Remesh 도구 5종

| 도구 | 용도 | 위치 |
|------|------|------|
| **Mesh Cleaner 2** | 중복 버텍스·뒤집힌 노멀 원클릭 정리 | 애드온 설치 후 사이드바 |
| **Voxel Remesh** | 폴리곤 균일화 | Object Data Properties |
| **Quad Remesh (Ctrl+R)** | 쿼드 토폴로지 변환 (애니메이션용) | Sculpt Mode |
| **Decimate Modifier** | 고폴리 감량 | Modifier → Generate |
| **QRemeshify** | 삼각형→쿼드 자동 변환 | 애드온 설치 후 사이드바 |

### 실습 순서

1. **Week 5 .blend 열기 → Statistics 켜서 폴리곤 수 기록**
   - Viewport Overlay → Statistics 체크
   - 현재 폴리곤 수를 메모 (Before)

2. **Mesh Cleaner 2 실행** (가장 먼저)
   - 중복 버텍스, 빈 구멍, 뒤집힌 노멀 원클릭 정리

3. **Decimate Modifier 추가 → Ratio 0.5**
   - Properties → Modifier → Add Modifier → Generate → Decimate
   - 형태가 무너지기 직전까지 Ratio 낮추기

4. **Sculpt Mode → Ctrl+R → Quad Remesh** (선택)
   - Voxel Size 0.02~0.05 권장

5. **QRemeshify** (삼각형이 많은 경우)
   - 트라이앵글 → 쿼드 자동 변환

6. **Before/After 비교 스크린샷**
   - Numpad 1 고정 앵글로 전후 동일 앵글 촬영 (Ctrl+F3)

> ⚠️ **체크:** Remesh 후 Material Preview(Z)로 쉐이더 깨짐 없는지 확인. 노멀이 이상하면 Edit Mode → Mesh → Normals → Recalculate Outside.
...
- 핵심 키워드: AI 메쉬 Remesh 정리 · 재질 시스템 · Principled BSDF · 노드 편집
- 예상 시간: ~3시간

### 실습 단계

#### 1. AI 메쉬 Remesh 정리 — 작업 가능 컨디션 만들기

Week 5에서 AI(Meshy/Tripo)로 뽑은 메쉬는 토폴로지가 엉망이에요. Remesh로 '작업 가능한 상태'를 먼저 만들어요.
...
- Week 5 .blend 파일 열기 — Statistics 켜서 폴리곤 수 확인
- Mesh Cleaner 2 실행 — 중복 버텍스 · 뒤집힌 노멀 정리
- Decimate Modifier 추가 → Ratio 0.5로 폴리곤 절반 감량
- Sculpt Mode → Ctrl+R → Quad Remesh (선택)
- Before/After 폴리곤 수 비교 스크린샷 (Statistics, Numpad 1 고정)

#### 2. Material 할당

리메시로 정리된 메쉬에 옷을 입히듯 재질을 입혀요. 같은 로봇이라도 재질 하나로 장난감이 될 수도, 군용 장비가 될 수도 있어요.
...
- `Ctrl + R` (Sculpt Mode): Quad Remesh 실행
...
- 과제명: 로봇/캐릭터 재질 적용
- 설명: Week 5 모델을 Remesh 정리 후, 3가지 이상 서로 다른 재질을 적용하고 렌더 이미지 제출.
- 제출 체크:
  - Remesh 전후 폴리곤 수 비교 스크린샷 (Statistics)
...
- Remesh 후 쉐이더 깨짐 → Mesh Cleaner 2 재실행 또는 Edit Mode → Normals → Recalculate Outside
- Decimate Ratio 너무 낮추면 형태 붕괴 → 0.3~0.5에서 멈추기
...
- [ ] **[Remesh]** Week 5 .blend 열기 → Statistics로 폴리곤 수 확인 (Before)
- [ ] **[Remesh]** Mesh Cleaner 2 실행 — 노멀/버텍스 정리
- [ ] **[Remesh]** Decimate Modifier로 폴리곤 감량 (형태 유지 확인)
- [ ] **[Remesh]** Before/After 비교 스크린샷 저장 (Numpad 1 고정 앵글)
