...
![Mirror Modifier - 한쪽만 편집하면 반대쪽이 자동으로 따라온다](../../course-site/assets/images/week03/mirror-modifier.png)
...
- 절반을 지우고 Mirror 추가하기 (`Edit Mode → 오른쪽 절반 선택 → X → Vertices`)
- Clipping 켜기 — 중심선 버텍스가 반대편으로 넘어가지 않게 고정
- 한쪽만 Extrude해서 자동 대칭 확인
- 중심선이 벌어지면 `S + X + 0` 써보기 (X축 0으로 강제 정렬)

핵심 파라미터

| 파라미터 | 의미 |
|----------|------|
| Axis X / Y / Z | 어떤 축을 기준으로 대칭할지 (좌우=X, 앞뒤=Y) |
| Clipping | 중심선 버텍스가 경계를 넘지 않게 잠금 |
| Merge | 중심 버텍스를 하나로 합쳐 경계 없이 만들기 |

#### 3. 곡면과 두께

각진 박스를 더 부드럽게 만들거나 납작한 면에 두께를 줄 수 있어요. 큰 실루엣을 빠르게 다듬을 때 많이 써요.

![Subdivision Surface - 레벨이 높아질수록 표면이 부드러워진다](../../course-site/assets/images/week03/subdivision-surface.png)
...
체크해볼 것 — Subdivision Surface

- `Ctrl+1/2/3`으로 Viewport Level 바꿔보기 (숫자가 클수록 더 부드럽고 무거워짐)
- `Shift+E`로 날카롭게 유지할 Edge에 Crease 주기 (값이 1.0이면 Subdivision에도 날카롭게 유지)
- `Shade Smooth` 함께 적용하기 (Object Mode 우클릭)

| 파라미터 | 의미 | 추천 값 |
|----------|------|---------|
| Levels Viewport | 작업 중 화면 분할 수 | 1~2 |
| Levels Render | 렌더링 시 분할 수 | 2~3 |

체크해볼 것 — Solidify

- Plane에 Solidify 추가 → Thickness `0.05` 입력
- Offset을 `-1 / 0 / 1`로 바꿔가며 두께 방향 비교
- Even Thickness 켜고 끄면서 곡면에서 두께 차이 확인

| 파라미터 | 의미 |
|----------|------|
| Thickness | 두께 값 |
| Offset | 두께가 자라는 방향 (-1 안쪽, 0 중앙, 1 바깥) |
| Even Thickness | 곡면에서도 균일한 두께 유지 |

#### 4. 반복과 구멍

도장을 찍듯 같은 부품을 반복하거나, 블록을 빼내듯 구멍을 만들 수 있어요. 디테일을 빠르게 늘릴 때 유용해요.

![Array로 반복 구조, Boolean Difference로 구멍 낸 결과](../../course-site/assets/images/week03/array-boolean-detail.png)
...
- Array 추가 → Count `5`, Relative Offset X `1.5`로 설정
- Y·Z 방향으로도 바꿔서 가로/세로/높이 반복 비교
- Array 2개 쌓기 (X축 + Y축) → 격자 배열 만들어보기

| 파라미터 | 의미 |
|----------|------|
| Fixed Count | 개수 직접 지정 |
| Relative Offset | 오브젝트 크기 기준 간격 (1.0 = 딱 붙음) |
| Constant Offset | Blender 단위 기준 고정 간격 |

체크해볼 것 — Boolean

- 바디 오브젝트에 Boolean Difference 추가
- Object에 커터 Cylinder/Cube 지정
- 커터 오브젝트 숨기기(H) → 구멍이 생겼는지 확인
- Union과 Intersect도 각각 바꿔서 차이 비교

| 연산 | 결과 |
|------|------|
| Union | A + B 합치기 |
| Difference | A에서 B 형태를 제거 |
| Intersect | 겹치는 부분만 남김 |

> ⚠️ Boolean이 이상할 때: 커터가 충분히 겹쳤는지 확인, `Ctrl+A > All Transforms` 먼저 적용

#### 5. 필수 추가 Modifier

핵심 5개 다음으로 바로 써먹기 좋은 보조 Modifier예요. `Ctrl+B`는 부분 수정, Bevel Modifier는 전체 정리, Weighted Normal은 음영 정리라고 나눠 생각하면 이해가 쉬워요.

![Bevel Modifier + Weighted Normal 적용 전후 비교](../../course-site/assets/images/week03/bevel-weighted-normal.png)

배울 것

- 필수 추가 Modifier를 안다

체크해볼 것 — Bevel Modifier

- Bevel Modifier 추가 → Amount `0.02`, Segments `2`로 시작
- Amount를 `0.01 ~ 0.1` 범위에서 바꿔서 모서리 느낌 비교
- Limit Method를 Angle로 설정 → 각도 조절하며 어떤 모서리에 적용되는지 확인
- `Ctrl+B`(Edit Mode 직접 베벨)와 Bevel Modifier 차이 느껴보기

| 파라미터 | 의미 |
|----------|------|
| Amount | 베벨 폭 |
| Segments | 분할 수 (많을수록 둥글어짐) |
| Limit Method: Angle | 특정 각도 이상 모서리에만 적용 |

체크해볼 것 — Weighted Normal

- Bevel Modifier 아래에 Weighted Normal 추가
- `Object Properties → Auto Smooth` 활성화
- Weighted Normal 켜고 끄면서 음영 변화 확인
- Face Influence 옵션도 켜서 차이 비교
