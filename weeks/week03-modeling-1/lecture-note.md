...
레퍼런스를 보면서 Tab으로 Edit Mode에 들어가 Extrude로 덩어리를 뽑고, Loop Cut으로 나누고, Inset과 Bevel로 다듬는 게 기본 흐름이에요.
...
- Shift+A → Image → Reference로 참고 이미지 불러오기 (Data 탭에서 Opacity 조절)
- Extrude (E)로 덩어리 뽑기 (면 선택 후 E → G+축 고정으로 방향 제어)
- Loop Cut (Ctrl+R)으로 분할선 추가하기 (스크롤로 루프 개수 조절)
- Inset (I)과 Bevel (Ctrl+B)로 패널·모서리 다듬기 (Bevel은 스크롤로 세그먼트 추가)

#### 3. Mirror

X축을 기준으로 반쪽을 대칭 복사해요. 한쪽만 만들면 작업량이 반으로 줄어요. Clipping을 켜면 중앙 정점이 넘어가지 않게 붙잡아줘요.

![Mirror](../../course-site/assets/images/week03/mirror-modifier.png)
...
- 절반 지우고 Add Modifier → Mirror 추가하기 (Axis: X 확인)
- Clipping 옵션 켜기 (중심선 버텍스가 넘어가지 않게 고정)
- 한쪽만 Extrude해서 대칭 반영 확인하기 (중심선 벌어지면 S+X+0으로 정렬)

#### 4. Subdivision Surface

면을 잘게 쪼개서 표면을 둥글고 매끄럽게 만들어요. 단, 전부 뭉개지는 걸 막으려면 날카운 모서리 옆에 Support Loop (Ctrl+R)를 추가하거나 Edge Crease (Shift+E)를 줘야 해요.

![Subdivision Surface](../../course-site/assets/images/week03/subdivision-surface.png)
...
- Subdivision Surface 추가하고 Ctrl+1/2/3으로 레벨 비교하기 (Viewport와 Render 레벨 차이 확인)
- 날카운 모서리 옆에 Support Loop 추가하기 (Ctrl+R로 모서리 근처에 루프 2개 — 형태가 살아나는 것 확인)
- Shift+E로 Edge Crease 적용 후 Support Loop와 비교하기 (두 방법의 차이를 눈으로 확인)

#### 5. Solidify

종이처럼 얇은 면(Plane)에 두께를 줘서 판 형태로 만들어요. 로봇의 장갑판이나 외장 패널을 만들 때 유용해요.

![Solidify](../../course-site/assets/images/week03/solidify-modifier.png)
...
- Plane에 Solidify 추가하고 Thickness 조절하기 (양수/음수로 방향 변화 확인)
- Offset 값을 -1 / 0 / 1로 바꿔보기 (두께가 안쪽/양쪽/바깥쪽으로 붙는 차이 확인)

#### 6. Array

같은 부품을 일정 간격으로 복제해요. Count로 개수, Offset으로 간격을 조절하면 반복 파츠를 한 번에 만들 수 있어요.

![Array](../../course-site/assets/images/week03/array-modifier.png)
...
- Array 추가하고 Count를 5로 설정하기 (Relative Offset으로 간격 조절)
- Constant Offset으로 X/Y/Z 방향 바꿔보기 (Relative vs Constant 차이 비교)
...
- `Shift + A`: Add 메뉴 (Image → Reference로 참고 이미지 추가)
- `1 / 2 / 3`: Vertex / Edge / Face 선택 모드 전환
- `E`: Extrude (면/선 돌출)
- `Alt + E`: Extrude Along Normals (면 방향으로 돌출)
...
- [[실습] 민트 로봇 레퍼런스 이미지 설정](https://youtu.be/-U82eI3eiQ0)
- [[실습] 민트 로봇 헤드 Mirror 모델링](https://youtu.be/FIY9RBLdYcA)
